...
exports.registerEmployee = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (user) {
      ...
      if (id === "") {
        ...
        console.log(typeof user?.joinDate);
        res.status(200).json({
          success: true,
          user,
        });
...
exports.loginUser = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!userName || !password) {
      return next(
        ErrorHandler("USERNAME OR PASSWORD REQUERED", 400, res, next)
      );
    }
    ...
    if (!user) {
      return next(
        ErrorHandler("PLEASE ENTER VALID USERNAME OR PASSWORD", 400, res, next)
      );
    }
...
export const getLoginHistory = catchAsyncError(
  async (req: Request, res: Response) => {
    ...
    const date = new Date(
      user.loginHistory[user.loginHistory.length - 2].timestamp
    );
    ...
    res.json({
      loginHistory: `${dateH}/${dateM}/${dateS}`,
      login: user.loginHistory,
    });
  }
);
