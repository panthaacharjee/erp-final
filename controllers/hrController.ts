...
exports.employeeDetails = catchAsyncError(async (req:Request, res:Response, next:NextFunction) => {
    ...
    const data = {
      ...
      join: `${joinDate.getDate()}, ${monthArray[joinDate.getMonth()]} ${joinDate.getFullYear()}`,
      phone: user?.personalInformation?.phone && user?.personalInformation.phone.length <=10 ? `+880${user?.personalInformation.phone}` :  user?.personalInformation.phone,
...
exports.allEmployee = catchAsyncError(async (req:Request, res:Response, next:NextFunction) => {
  ...
  const namesArray = typeof nameFilter === 'string' 
    ? nameFilter.split(',') 
    : Array.isArray(nameFilter) 
      ? nameFilter 
      : [];

  const idsArray = typeof idFilter === 'string' 
    ? idFilter.split(',') 
    : Array.isArray(idFilter) 
      ? idFilter 
      : [];

   const categoriesArray = typeof categoryFilter === 'string' 
    ? categoryFilter.split(',') 
    : Array.isArray(categoryFilter) 
      ? categoryFilter 
      : [];
 
  if(namesArray.length>0 && idsArray.length>0 && categoriesArray.length>0){
    const users = await User.find({account:"Regular"})
    const filterUsers = await User.find({name:{$in: namesArray}, employeeId:{$in:idsArray}, category:{$in: categoriesArray}, account:"Regular"});
    res.status(200).json({
        success: true,
        users,
        filterUsers
...
exports.salaryCreate = catchAsyncError(async (req:Request, res:Response, next:NextFunction) => {
  ...
  const missingAbsenteeism = data.some(item=>item.Absenteeism=== undefined)
  ...
  if (missingPresent) {
    return next(ErrorHandler("PRESENT FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingWeekend) {
    return next(ErrorHandler("WEEKEND FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingHolidays) {
    return next(ErrorHandler("HOLIDAYS FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingAbsent) {
    return next(ErrorHandler("ABSENT FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingCL) {
    return next(ErrorHandler("CL FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingSick) {
    return next(ErrorHandler("SICK FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingEarn) {
    return next(ErrorHandler("EARN FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingMaternity) {
    return next(ErrorHandler("MATERNITY FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingOTHours) {
    return next(ErrorHandler("OT HOURS  FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingOTRate) {
    return next(ErrorHandler("OT RATE FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingOTAmount) {
    return next(ErrorHandler("OT AMOUNT FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingAdvanced) {
    return next(ErrorHandler("ADVANCED FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingAbsenteeism) {
    return next(ErrorHandler("ABSENTEEISM FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingLoan) {
    return next(ErrorHandler("LOAN FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingPF) {
    return next(ErrorHandler("PF FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingOthers) {
    return next(ErrorHandler("OTHERS FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  if (missingIncomeTax) {
    return next(ErrorHandler("INCOME TAX FIELD IS MISSING IN ONE OR MORE ROWS", 400, res, next));
  }
  ...
  if (data.length > regularUsers.length) {
    return next(ErrorHandler("EXCEL CONTAYS EMPLOYEES NOT IN DATABASE", 404, res, next));
  }
  ...
  if (data.length < totalRegularUsers) {
    ...
    const userMap = new Map(regularUsers.map(user => [user.employeeId, user._id]));
    ...
    if (existingSalaries.length > 0) {
      ...
      const existingEmployees = regularUsers
        .filter((user:any) => !existingUserIds.some(id => id.equals(user._id)))
        .map(user => user.employeeId);
      
      if(existingEmployees.length>0){
         return next(ErrorHandler(
          `SALARIES EXIST! BUT NOT EXIST EMPLOYEES: ${existingEmployees.join(', ')} ON ${date}`,
          400,
          res,
          next
        ));
  ...
  const bulkOps = data.map(item => ({
    insertOne:{
      document:{
        ...
        date: date,
        salary:{
          main:regularUsers.filter(i=>i.employeeId===item.ID)[0]?.mainSalary,
          basic:regularUsers.filter(i=>i.employeeId===item.ID)[0]?.salary?.basic,
          home:regularUsers.filter(i=>i.employeeId===item.ID)[0]?.salary?.home,
          food:regularUsers.filter(i=>i.employeeId===item.ID)[0]?.salary?.food,
          medical:regularUsers.filter(i=>i.employeeId===item.ID)[0]?.salary?.medical,
          conveyance:regularUsers.filter(i=>i.employeeId===item.ID)[0]?.salary?.conveyance,
          special:regularUsers.filter(i=>i.employeeId===item.ID)[0]?.salary?.special
        },
        attendence:{
...
/* ===================================================================================================== */
exports.singleSalary = catchAsyncError(async(req:Request, res:Response, next:NextFunction)=>{
    const {
      date,
      id,
      present,
...
/* ===================================================================================================== */
exports.pdfSalary = catchAsyncError(async(req:Request, res:Response, next:NextFunction)=>{
    ...
    employeesSalaryList.map(async (employee) => {
      ...
      if (typeof employee.user === 'object' && 'employeeId' in employee.user) {
          const monthHalf = ['Jan', "Feb", "Mar", "Apr", "May", "Jun", "Jul","Aug", "Sep", "Oct", "Nov", "Dec"]
          const monthFull = ['January', "February", "March", "April", "May", "June", "July", "Augest", "September", "October", "November", "December"]
          
          const joinDate = new Date(employee.user.joinDate)
          const dateJoin = `${joinDate.getDate()}-${monthHalf[joinDate.getMonth()]}-${joinDate.getFullYear()}`
          
          const salaryDate = `${monthFull[new Date(date).getMonth()]}-${new Date(date).getFullYear()}`

          const totalSalary = employee.salary.main + employee.salary.special + employee.ot.amount - employee.deduction.advanced - employee.deduction.absenteeism - employee.deduction.loan - employee.deduction.pf - employee.deduction.tax - employee.deduction.tax
